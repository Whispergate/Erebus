...
- **Post-run cleanup uses an out-of-process watcher** rather than an in-process `child.on('exit')` handler. The Electron main process exits seconds after the fake wizard advances to Finish, which would kill any in-process cleanup callback before the loader finishes. After spawning the loader, main.js writes a tiny self-deleting batch file to `%TEMP%\inst-clean-<uuid>.bat` and launches it detached via `cmd.exe`. The batch polls `tasklist` for the loader PID, waits for the loader to exit, then `rd /s /q`'s the inst-tree and `del`s itself via the `(goto) 2>nul & del "%~f0"` idiom. Also avoids the Windows file-lock problem where a running PE can't be deleted by its own parent.
- The watcher adds a single `cmd.exe` child to the process tree during cleanup. Its command line is `cmd /c inst-clean-<uuid>.bat <pid> <tmpdir>` - visible in Process Explorer and Sysmon Event ID 1. If the victim's telemetry specifically flags short-lived `cmd.exe` spawns from Electron-style parents, the watcher is a signal.
- If the victim reboots (or force-kills the Electron main) between Install click and loader exit, the cleanup watcher is killed mid-wait and the inst-tree persists until next manual `%TEMP%` cleanup. There is no secondary fallback (no `MoveFileEx MOVEFILE_DELAY_UNTIL_REBOOT`, no scheduled task).
...
- Test on a Windows 10 sandbox to confirm the cleanup watcher actually fires - after the fake wizard closes, `%TEMP%\inst-<uuid>\` and `%TEMP%\inst-clean-<uuid>.bat` should both be gone within a few seconds of the loader exiting.
...
- Add a reboot-survival fallback for the cleanup watcher: when the initial `rd /s /q` fails (loader still has files locked), fall back to scheduling the inst-tree for deletion on next reboot via `MoveFileEx(MOVEFILE_DELAY_UNTIL_REBOOT)`. This requires shelling to a tiny PE helper since Node can't make kernel32 calls directly without a native module.
- Consider replacing the `cmd.exe` watcher with a compiled mingw-w64 helper exe that does the PID polling + rm-rf natively. Removes the `cmd.exe` process-tree signal and supports MoveFileEx fallback.
